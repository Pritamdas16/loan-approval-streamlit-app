...
st.info("👉 Please fill **both Applicant Details and Financial Details** before checking eligibility.")
...
with tab1:
    ...
    with col1:
        employment_status = st.selectbox(
            "Employment Status", ["Select", "Salaried", "Self-employed", "Unemployed"]
        )
        loan_purpose = st.selectbox(
            "Loan Purpose", ["Select", "Home", "Car", "Education", "Personal"]
        )
    ...
    with col2:
        ...
        employer_category = st.selectbox(
            "Employer Category",
            ["Select", "Government", "MNC", "Private", "Unemployed"]
        )
...
with tab2:
    ...
    with col3:
        ...
        credit_score = st.number_input(
            "Credit Score", min_value=0, max_value=900
        )
...
if st.button("🔍 Check Loan Eligibility", use_container_width=True):
    ...
    if employer_category == "Select":
        missing_fields.append("Employer Category")

    # Financial validation
    if applicant_income <= 0:
        missing_fields.append("Applicant Income")
    if loan_amount <= 0:
    ...
    if prediction == 1:
        st.markdown(
            f"<div class='result-approve'><h2>✅ Loan Approved</h2>"
            f"<p>Confidence Score: <b>{display_conf:.2f}%</b></p></div>",
            unsafe_allow_html=True
        )
    else:
        st.markdown(
            f"<div class='result-reject'><h2>❌ Loan Rejected</h2>"
            f"<p>Confidence Score: <b>{display_conf:.2f}%</b></p></div>",
            unsafe_allow_html=True
        )
